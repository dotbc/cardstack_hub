const { declareInjections, getOwner, setOwner } = require('@cardstack/di')
...
module.exports = declareInjections(
  ...
  class PluginLoader {
    ...
    async configuredPlugins (configModels) {
      ...
      let a = new ConfiguredPlugins(
        installed,
        await this.installedFeatures(),
        configs
      )
    ...
    async _crawlPlugins (
      ...
      log.trace(
        'plugin crawl dir=%s, includeDevDependencies=%s, breadcrumbs=%j',
        dir,
        includeDevDependencies,
        breadcrumbs
      )
      ...
      if (dupeModule) {
        let msg = action =>
          `The plugin module name '${json.name}' has already been loaded from the module path ${dupeModule.attributes.rawDir}, ${action} load of module at path ${dir}.`
      ...
      seen[realdir] = true
      let moduleRoot = path.dirname(
        await resolve(packageJSON, { basedir: this.project.path })
      )

      if (
        json.keywords &&
        json.keywords.includes('cardstack-plugin') &&
        json['cardstack-plugin']
      ) {
        if (json['cardstack-plugin']['api-version'] !== 1) {
          log.warn(
            `%s has some fancy cardstack-plugin.version I don't understand. Trying anyway.`,
            realdir
          )
        }
      ...
      let deps = json.dependencies
        ? Object.keys(json.dependencies).map(dep => ({
            dep,
            type: 'dependencies'
          }))
        : []
      if (includeDevDependencies && json.devDependencies) {
        deps = deps.concat(
          Object.keys(json.devDependencies).map(dep => ({
            dep,
      ...
      if (json['cardstack-plugin']) {
        ...
        if (dirs) {
          deps = deps.concat(
            dirs.map(dir => ({
              dep: path.resolve(moduleRoot + '/' + dir),
              type: 'in-repo-plugins'
            }))
          )
        }
      }
      ...
      for (let { dep, type } of deps) {
        let childDir = path.dirname(
          await resolve(dep + '/package.json', { basedir: realdir })
        )
        ...
        await this._crawlPlugins(
          ...
          seen,
          includeDevDependencies,
          breadcrumbs.concat({ id: json.name, type })
        )
      }
    }
...
class ConfiguredPlugins {
  constructor (installedPlugins, installedFeatures, configs) {
    ...
    installedPlugins.forEach(plugin => {
      ...
      if (config) {
        copied.attributes = Object.assign(
          {},
          plugin.attributes,
          config.attributes
        )
        ...
        copied.relationships = Object.assign(
          {},
          plugin.relationships,
  ...
  lookupFeatureAndAssert (featureType, fullyQualifiedName) {
    return this._instance(
      this._lookupFeatureAndAssert(featureType, fullyQualifiedName)
    )
  }
  ...
  featuresOfType (featureType) {
    ...
    if (!typeSet) {
      ...
      return Object.values(typeSet).filter(
        feature =>
          this._plugins[feature.relationships.plugin.data.id].attributes.enabled
      )
    }
  }
    ...
    if (typeSet) {
      ...
      if (feature) {
        if (
          ...
          throw new Error(
            `You're trying to use ${featureType} ${fullyQualifiedName} but the plugin ${feature.relationships.plugin.data.id} is not activated`
          )
        }
      } else {
        ...
        if (this._plugins[moduleName]) {
          throw new Error(
            `You're trying to use ${featureType} ${fullyQualifiedName} but no such feature exists in plugin ${moduleName}`
          )
        } else {
          throw new Error(
            `You're trying to use ${featureType} ${fullyQualifiedName} but the plugin ${moduleName} is not installed. Make sure it appears in the dependencies section of package.json`
          )
        }
      }
    } else {
...
function summarize (plugins, features) {
  return plugins
    .map(p => {
      let pluginFeatures = features.filter(
        f => f.relationships.plugin.data.id === p.id
      )
  ...
  while (queue.length > 0) {
    ...
    if (deps) {
      for (let dep of deps) {
        if (!configs.get(dep)) {
          log.debug(
            'Activating plugin %s because its used by %s',
            dep,
            pluginName
          )
...
function dependencyGraph (installed) {
  ...
  for (let plugin of installed) {
    for (let breadcrumbs of plugin.attributes.includedFrom) {
      ...
      if (!parent || parent.type !== 'dependencies') {
        continue
      }
